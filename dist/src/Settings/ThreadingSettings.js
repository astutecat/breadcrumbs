...
import { ARROW_DIRECTIONS, DEFAULT_SETTINGS, DIRECTIONS, ILLEGAL_FILENAME_CHARS, } from "../constants";
import { fragWithHTML, subDetails } from "./BreadcrumbsSettingTab";
export function addThreadingSettings(plugin, cmdsDetails) {
    ...
    new Setting(threadingDetails)
        ...
        text.inputEl.onblur = async () => {
            ...
            if (ILLEGAL_FILENAME_CHARS.some((char) => value.includes(char))) {
                new Notice(`File name cannot contain any of these characters: ${ILLEGAL_FILENAME_CHARS.join(" ")}`);
                text.setValue(settings.threadingTemplate);
            }
            else {
...
//# sourceMappingURL=ThreadingSettings.js.map