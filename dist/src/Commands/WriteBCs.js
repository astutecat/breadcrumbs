...
export async function writeBCToFile(plugin, currFile) {
...
export async function writeBCsToAllFiles(plugin) {
    if (!plugin.settings.showWriteAllBCsCmd) {
        new Notice("You first need to enable this command in Breadcrumbs' settings.");
        return;
    }
        ...
        if (window.confirm("Are you sure? You have been warned that this operation will attempt to update all files with implied breadcrumbs.")) {
            if (window.confirm("For real, please make a back up before.")) {
                ...
                if (problemFiles.length) {
                    new Notice("Some files were not updated due to errors. Check the console to see which ones.");
                    warn({ problemFiles });
                }
            }
        }
...
//# sourceMappingURL=WriteBCs.js.map