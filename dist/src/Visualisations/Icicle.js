...
export const icicle = (graph, currFile, modal, width, viewHeight) => {
    const flatAdj = dfsFlatAdjList(graph, currFile.basename);
    console.log({ flatAdj });
    const hier = d3.stratify()(flatAdj);
    console.log({ hier });
    const format = d3.format(",d");
    const color = d3.scaleOrdinal(d3.quantize(d3.interpolateRainbow, hier.children.length + 1));
    const partition = (data) => {
        const root = d3
            .hierarchy(data)
            .sum((d) => d.value)
            .sort((a, b) => b.height - a.height || b.value - a.value);
        return d3.partition().size([viewHeight, ((root.height + 1) * width) / 3])(root);
    };
    ...
    cell.append("title").text((d) => `${d.ancestors().map((d) => d.data.data.name)
    // .reverse()
    // .join("/")}\n${format(d.value)
    }`);
