...
import { addHNsToGraph, getHierarchyNoteItems } from "./AlternativeHierarchies/HierarchyNotes/HierarchyNotes";
import { addJugglLinksToGraph, getJugglLinks } from "./AlternativeHierarchies/JugglLinks";
...
import { BC_ALTS, BC_DV_NOTE, BC_FOLDER_NOTE, BC_LINK_NOTE, BC_REGEX_NOTE, BC_TAG_NOTE, BC_TRAVERSE_NOTE, dropHeaderOrAlias, splitLinksRegex } from "./constants";
...
import { addNodesIfNot, buildObsGraph, getReflexiveClosure, getSourceOrder, getTargetOrder, populateMain } from "./Utils/graphUtils";
import { iterateHiers } from "./Utils/HierUtils";
import { getBaseFromMDPath, getDVApi, getDVBasename } from "./Utils/ObsidianUtils";
...
export async function buildMainG(plugin) {
        ...
        if (hierarchyNotes.length) {
            for (const noteOrFolder of hierarchyNotes) {
                if (noteOrFolder.endsWith("/")) {
                    const folder = app.vault.getAbstractFileByPath(normalizePath(noteOrFolder));
...
export function buildClosedG(plugin) {
...
export async function refreshIndex(plugin) {
    var _a;
